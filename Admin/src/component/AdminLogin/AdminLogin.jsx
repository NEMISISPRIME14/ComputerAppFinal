...
export default function AdminLogin({ }) {
  let { saveLoginData } = useContext(AdminAuthContext)
  let { register, handleSubmit, formState: { errors } } = useForm()
  let navigate = useNavigate()
  let onSubmit = async (data) => {
    try {
      let response = await axios.post('http://localhost:3000/admin/loginadmin', data);
        ...
        <div className="row w-100">
          <div className="col-md-6 col-10 bg-light mx-auto p-4 rounded-4">
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <button type="submit" className="btn btn-warning text-uppercase text-white fw-bold fs-5 my-3 w-100">LogIn</button>
            </form>
          </div>
        </div>
      </div>
