...
export default function AcceptedAppointments() {
    ...
    let [acceptedAppointments, setAcceptedAppointments] = useState([])
    ...
    useEffect(() => {
        if (AppointmentsData) {
            let acceptedAppointments = AppointmentsData.filter(a => a.appointment_status === 'accepted')
            setAcceptedAppointments(acceptedAppointments)
        }
    }, [AppointmentsData])
return (
    <>
        <h2 className="text-center text-uppercase fw-bold h1 mb-5">Accepted Appointments</h2>
                        ...
                        acceptedAppointments.map((appointment, index) => {
                            ...
                            return (
                                <tr key={index} className="text-center align-middle border border-secondary">
                                    ...
                                    <td className='bg-transparent text-light'>
                                        <button className="btn btn-danger me-2" onClick={() => onClickDeleteAppointment(appointment.id)}><i className="fa-solid fa-trash"></i></button>
                                    </td>
                                </tr>
                            );
                        })
